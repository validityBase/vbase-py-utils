...
- Regenerates `requirements/tools.txt` and `requirements/dev.txt`; the workflow fails if the committed lock files differ.
...
- Installs `requirements/dev.txt` with Python 3.11 and `require-hashes: true`.
- Runs `pylint $(git ls-files '*.py')`.

### `.github/workflows/run-unit-tests.yml`
- Runs on pushes to all branches, including branch names containing `/`.
- Checks out the repository with the pinned `actions/checkout` action.
- Installs `requirements/dev.txt` with Python 3.11 through `setup-python-deps@v1` and `require-hashes: true`.
- Installs the package locally with `python -m pip install --no-deps --no-build-isolation -e .`.
